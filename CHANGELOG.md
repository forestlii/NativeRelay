...
- Samples (IMGUI, zero extra dependencies): **Basic Mock Demo** (concurrent out-of-order dispatch on the main thread) and **ASR Demo** (simulated speech→text driving a fake dialogue).
- Docs: `docs/architecture.md` (layered diagram + one-request sequence diagram); full `README.md` (install, 30-second quick start, API, how to plug in a real native channel).
