...
用 Lua 驱动桥是一样的——所有业务调用，以及成功 / 失败 / 错误码处理，都能写在 Lua 里。
一份开箱即用的样板（Lua 薄封装 + 必需的 xLua 生成配置 + 纯 Lua 业务样例）在配套仓库：
**[NativeRelay-Native / `examples/xlua/`](https://github.com/forestlii/NativeRelay-Native/tree/main/examples/xlua)**。
...
local bridge  = NR.MainThreadDispatcher.Instance:CreateBridge(channel, 5.0)  -- 实例方法用冒号 :

-- onResult 在【主线程】触发，所以（非线程安全的）Lua VM 可安全使用。
bridge:Request(myCommand, payload, function(code, data)
    if code == RelayCode.Timeout then        -- 超时
    elseif code == 1 then                    -- 成功：用 data（文本/路径）
    else                                     -- 你的业务错误码
    end
end)
```

> **IL2CPP / AOT 大坑：** 把 Lua 函数当 C# 的 `Action<int,string>` onResult 传，真机上会抛异常，
> 除非该委托登记进 `[CSharpCallLua]` 并重新生成 xLua 代码。样板里的 `NativeRelayXLuaConfig.cs`
> 正是干这个的——别跳过。
